...
if($loggedIn && $_SERVER["REQUEST_METHOD"] == "POST")
{
    if(isset($_POST["submit"]))
    {
        ...
        */
        ...
        $admin_exists = exist_user($conn,$user_name);
        $admin_added = false;
    ...
    if(isset($_POST["edit"]))
    {
        ...
        $id_if_admin_exists = exist_user($conn,$username);

        if(!$id_if_admin_exists || $id == $id_if_admin_exists)
        {
            $updateSql = "UPDATE `users` SET
            `user_name` = '$user_name',
            `user_fullname` = '$full_name',
            `user_password` = '$hash' WHERE `users`.`user_id` = '$id';";
            ...
            echo '<div class="my-0 alert alert-danger alert-dismissible fade show" role="alert">
            <strong>Error!</strong> Same User Name  already exists
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>';
        }
        ...
        <table class="table table-hover table-bordered">
            ...
            <tr>
                ...
                <button class="button edit-button " data-link="<?php echo $_SERVER['REQUEST_URI']; ?>" data-id="<?php
                                    echo $id;?>" data-full name="<?php
                                    echo $full_name;?>" data-user_name="<?php
                                    echo $user_name;?>"data-user_password="<?php
                                    echo $user_password;?>"
                                    >Edit</button>
